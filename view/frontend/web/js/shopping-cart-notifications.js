...
define([
    ...
    $.widget('mageWorkshop.shoppingCartNotifications', {
        ...
        _create: function() {
            ...
            // Add additional style for displaying messages with html code
            $.notify.addStyle("mageworkshop-message", {
        ...
        _subscribeToMessagesUpdate: function() {
            ...
            customerData.get('messages').subscribe(function (newValue) {
                ...
                if (!messagesWasRemoved && !$.isEmptyObject(newValue.messages) && self.options.disableDefaultNotification) {
                    messagesWasRemoved = true;
                ...
                if (newValue.messages) {
                    ko.utils.arrayForEach(newValue.messages, function (message) {
                        var type = self.options.messageTypesMapping.hasOwnProperty(message.type)
                            ...
                            : 'default';
                        $.notify(message.text, {
                            'className': self.options.messageTypesMapping[type],
                            'autoHideDelay': self.options.autoHideDelay,
                            'style': 'mageworkshop-message'
                        });
                    });
                }
                // Reset flag. It is needed for cleaning messages for another action on the same page
                messagesWasRemoved = false;
            });
        }
    });
